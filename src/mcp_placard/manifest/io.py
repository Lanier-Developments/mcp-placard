...
SUPPORTED_MANIFEST_VERSIONS = frozenset({"1.0", "2.0", "2.1", MANIFEST_VERSION})
...
upgrade step and still verifies (see ``models._drop_empty_kinds``). "2.1" differs
from "2.2" by the absence of ``injection_findings`` and ``ruleset_version``, handled
the same way: empty serializes as absent, and a missing ``ruleset_version`` selects
the pre-2.2 ``classification_hash`` shape (see ``build.classification_document``)."""
